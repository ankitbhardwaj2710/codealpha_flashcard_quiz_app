...
class ScoreService {
  ...
  /// Save Quiz Result
  Future<int> saveScore({required int correct, required int wrong}) async {
  ...
  Future<List<Map<String, dynamic>>> getAllScores() async {
    ...
    return await db.query('scores', orderBy: 'createdAt DESC');
  }
  ...
  Future<int> getTotalAttempts() async {
    ...
    final result = await db.rawQuery('SELECT COUNT(*) as total FROM scores');
